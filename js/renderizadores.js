...
(function () {
  ...
  function parsearPartidosExtraTexto(texto, nombreClubActivo, onIgnorada, onDuplicadoLiga) {
    ...
    (texto || "").split("\n").forEach(function (linea) {
      ...
      // Tolerante a que falte el espacio en UNO de los 2 lados del guion
      // separador (ej. "Ida Semifinal- Atlético Madrid" en vez de "Ida
      // Semifinal - Atlético Madrid"): con el espacio ausente en un solo
      // lado, la línea entera se perdía en silencio (menos de 3 partes) —
      // exactamente lo que le pasaba a la ronda "Ida Semifinal" de
      // Atlético Madrid/FC Barcelona (reporte usuario 2026-09-04, con el
      // resto de rondas de la misma línea pegadas SIN ese typo
      // funcionando bien). Sigue exigiendo espacio en AL MENOS un lado, así
  ...
  function _statTextoLeer(comp, ligaId, categoria) {
    ...
    switch (comp) {
      ...
      case "sce": return window.Estado.obtenerSceStatTexto(categoria);
  ...
  function _statTextoGuardar(comp, ligaId, categoria, texto) {
    ...
    switch (comp) {
      ...
      case "sce": window.Estado.guardarSceStatTexto(categoria, texto); break;
  ...
  // Playoffs (parsearChampionsPlayoffTexto/_championsTieRowHTML —
  // genéricos sobre un objeto `tie`, sin nada específico de Champions)
  // con su PROPIA clave de almacenamiento y su PROPIO detector de ronda
  // — la Copa NO tiene fase de grupos que alimente "esperas" como
  // Champions: TODAS las rondas (Dieciseisavos, Octavos, Cuartos,
  // Semis, Final) las da el admin directamente en texto, ninguna se
  // infiere de la ronda anterior (ver `_copaRondaDesdeTexto`).
  ...
  // Los cruces de UNA ronda de Copa (Dieciseisavos, Octavos, Cuartos,
  // Semis o Final — LAS 5 se resuelven IGUAL, ninguna depende de la
  // anterior): cada club humano con partidos YA jugados esta ronda
  // aporta el suyo auto-computado (agregado ida+vuelta si aplica); el
  // resto sale ÍNTEGRAMENTE del texto libre que el admin pega para ESA
  // ronda concreta ("Equipo A 3-1 Equipo B",
  // parsearChampionsPlayoffTexto). PROHIBIDO inferir el emparejamiento
  // de una ronda a partir del ganador de la ronda anterior — petición
  // usuario 2026-09-04: "los cruces los doy yo en texto, elimina el
  // algoritmo de generarlos automáticamente" (se reportó con Octavos
  // generándose solo a partir de los ganadores de Dieciseisavos).
  function _copaRondaDesdeTexto(datos, rondaKey) {
    var equiposHumanos = _copaEquiposHumanos(datos);
    var ties = [];
    var indexPorHumano = {};

    equiposHumanos.forEach(function (e) {
      var t = _copaPlayoffHumano(datos, e, rondaKey);
      if (!t) return;
  ...
  // INDEPENDIENTES entre sí, cada una sale directamente del texto que
  // el admin pegó para ESA ronda (nunca del ganador de la anterior).
  function calcularCopaPlayoffTodasLasRondas(datos) {
    return {
      dieciseisavos: _copaRondaDesdeTexto(datos, "dieciseisavos"),
      octavos: _copaRondaDesdeTexto(datos, "octavos"),
      cuartos: _copaRondaDesdeTexto(datos, "cuartos"),
      semis: _copaRondaDesdeTexto(datos, "semis"),
      final: _copaRondaDesdeTexto(datos, "final")
    };
  }
  ...
  // ============================================================
  // 3c-quinquies-ter. TORNEO DE VERANO — 👥️ Humanos + ⛓️ Eliminatorias —
  // MISMO estilo EXACTO que la Copa Intercontinental de arriba (16 clubes,
  // Octavos → Cuartos → Semifinal → Final, TODAS a partido único con
  // prórroga y penaltis — petición usuario 2026-09-15: "Creame torneo de
  // verano, 16 equipos jugando eliminatorias como la copa del rey desde
  // Octavos-Cuartos-Semifinal y final a partido único con prórroga y
  // penaltis, con editor"). Sin fase de grupos, sin exclusiones de club.
  // ============================================================
  function _veranoEquiposHumanos(datos) {
    return datos.equipos.equipos || [];
  }
  ...
  function obtenerFormatoVeranoTexto() {
    var override = window.Estado ? window.Estado.obtenerFormatoOverride("verano") : "";
  ...
  // Cruce AUTO-COMPUTADO de un club humano en UNA ronda concreta — mismo
  // criterio EXACTO que _interPlayoffHumano, filtrando
  // competicion==="verano" y reutilizando el detector de ronda de Copa
  // (_copaPlayoffRondaCoincide, genérico por rondaKey/rondaTexto — no
  // depende de qué competición sea).
  function _veranoPlayoffHumano(datos, club, rondaKey) {
    var partidos = (window.Estado ? window.Estado.listarPartidosResueltos(datos) : []).filter(function (p) {
      if (!(p.jugado && p.resultado && p.competicion === "verano" && (p.local === club.id || p.visitante === club.id))) return false;
      return _copaPlayoffRondaCoincide(rondaKey, p.ronda || "");
    });
    if (!partidos.length) return null;
    var rivalId = null, golA = 0, golB = 0;
  ...
  // Los cruces de UNA ronda del Torneo de Verano (Octavos, Cuartos, Semis
  // o Final — LAS 4 se resuelven IGUAL, ninguna depende de la anterior) —
  // mismo criterio EXACTO que _interRondaDesdeTexto: cada club humano con
  ...
  function _veranoRondaDesdeTexto(datos, rondaKey) {
    var equiposHumanos = _veranoEquiposHumanos(datos);
    var ties = [];
    var indexPorHumano = {};

    equiposHumanos.forEach(function (e) {
      var t = _veranoPlayoffHumano(datos, e, rondaKey);
      if (!t) return;
      indexPorHumano[e.id] = ties.length;
      ties.push({
        equipoA: t.equipoA, equipoAObj: t.equipoAObj, golA: t.golA, golB: t.golB,
    ...
    var texto = window.Estado ? window.Estado.obtenerVeranoPlayoffTexto(rondaKey) : "";
    parsearChampionsPlayoffTexto(texto).forEach(function (it) {
      var objA = resolverRivalPorNombre(it.equipoA, datos, null);
      var objB = resolverRivalPorNombre(it.equipoB, datos, null);
      var idxHumanoA = objA && objA.mister ? indexPorHumano[objA.id] : undefined;
  ...
  // INDEPENDIENTES entre sí, cada una sale directamente del texto que el
  // admin pegó para ESA ronda.
  function calcularVeranoPlayoffTodasLasRondas(datos) {
    return {
      octavos: _veranoRondaDesdeTexto(datos, "octavos"),
      cuartos: _veranoRondaDesdeTexto(datos, "cuartos"),
      semis: _veranoRondaDesdeTexto(datos, "semis"),
      final: _veranoRondaDesdeTexto(datos, "final")
    };
  }

  function _renderizarVeranoEliminatorias(contenedor, datos, idClubActivo) {
    var rondas = calcularVeranoPlayoffTodasLasRondas(datos);

    VERANO_PLAYOFF_RONDAS.forEach(function (meta) {
      var ties = rondas[meta.key] || [];
      var bloque = document.createElement("div");
      bloque.className = "copa-club-block champions-playoff-ronda";
      var header =
        '<div class="copa-club-header">' +
        '<span class="copa-club-nombre">' + escapeHTML(meta.label) + "</span>" +
        '<button type="button" class="liga1ref-editar-btn" data-accion="editar-verano-playoff-inline" data-ronda="' +
        meta.key + '" data-club-id="' + (idClubActivo || "") + '" aria-label="Editar ' + escapeHTML(meta.label) + '">✏️</button>' +
        "</div>";
      var cuerpo;
      if (!ties.length) {
  ...
  // Editor inline de UNA ronda del cuadro ⛓️ Eliminatorias (PIN 646) —
  // mismo patrón exacto que pintarEditorInterPlayoff: las 4 rondas se
  // pegan igual, una línea por cruce, siempre marcador de partido único.
  function pintarEditorVeranoPlayoff(contenedor, idClubActivo, rondaKey) {
    var meta = VERANO_PLAYOFF_RONDAS.filter(function (r) { return r.key === rondaKey; })[0];
    if (!meta) return;
    var esOctavos = rondaKey === "octavos";
    contenedor.innerHTML = "";
    ...
    nota.textContent = esOctavos
      ...
      : "Pega el marcador (partido único, con prórroga/penaltis si hizo falta) de cada cruce, una línea por " +
        "eliminatoria — vale «-» o «vs»: «Equipo A 3-1 Equipo B» o «Equipo A 3 vs 1 Equipo B». Si un club " +
        "humano ya tiene sus propios partidos del Torneo de Verano jugados en esta ronda (Calendario extra), su " +
        "cruce se calcula solo y esta línea se ignora.";
    contenedor.appendChild(nota);

    var textarea = document.createElement("textarea");
    textarea.id = "verano-playoff-textarea";
    textarea.className = "admin-roadmap-textarea";
    textarea.rows = esOctavos ? 10 : 8;
    textarea.placeholder = esOctavos
      ? "Liverpool 2-1 RB Leipzig\nArsenal 3 vs 0 Sporting CP"
      : "Liverpool 1-0 Arsenal\nReal Madrid 2 vs 2 Villarreal";
    textarea.value = window.Estado ? window.Estado.obtenerVeranoPlayoffTexto(rondaKey) : "";
    contenedor.appendChild(textarea);
  ...
  // ============================================================
  // 3c-quinquies-quinquies. SUPERCOPA DE ESPAÑA — 👥️ Humanos +
  // ⛓️ Eliminatorias — MISMO estilo EXACTO que la Supercopa de Europa de
  // arriba (4 clubes, 2 rondas: Semifinal 2 vs 2 → Final 1 vs 1), pero
  // usando el compKey "supercopa" — petición usuario 2026-09-16: "Son 4
  // equipos jugando Semifinal y final a partido único" (la tarjeta 🏅
  // "Supercopa España" ya vivía en MENU_CLUB_BUILTIN desde antes, pero
  // caía al placeholder "todavía no está disponible" — nunca tuvo motor
  // propio). "supercopa" ya era un compKey reconocido (balón/color de
  // card comp-supercopa/Final forzada — ver _BALON_COMP_ALIAS, COMP_LABEL,
  ...
  // mismo criterio EXACTO que _uscPartidosDelClub. `competicion==="supercopa"`
  // ya sale normalizado de Calendario extra (_BALON_COMP_ALIAS:
  // "Supercopa España"/"Supercopa de España" tecleado por el admin
  // siempre cuadra con este ===).
  function _scePartidosDelClub(datos, clubId) {
    return (window.Estado ? window.Estado.listarPartidosResueltos(datos) : [])
      .filter(function (p) {
        return p.competicion === "supercopa" && (p.local === clubId || p.visitante === clubId);
      })
      .sort(function (a, b) {
        var ta = a.fecha ? new Date(a.fecha).getTime() : (a._fechaFallbackMs || 0);
        var tb = b.fecha ? new Date(b.fecha).getTime() : (b._fechaFallbackMs || 0);
  ...
  // Recorre, para cada club humano, sus propios partidos de Supercopa de
  // España ya jugados y suma goles/MVP/amarillas/rojas por jugador —
  // mismo criterio EXACTO que calcularUscStatsHumanos, con su PROPIO
  // acumulado (nunca comparte contador con ninguna otra competición).
  function calcularSceStatsHumanos(datos) {
    var acumulado = { pichichi: {}, mvp: {}, amarillas: {}, rojas: {} };
    ...
    _sceEquiposHumanos(datos).forEach(function (e) {
      var nombresPorId = {};
      obtenerJugadoresClub(e.id).forEach(function (j) { nombresPorId[j.id] = j.nombre; });

      _scePartidosDelClub(datos, e.id).filter(function (p) { return p.jugado; }).forEach(function (p) {
        var oponenteId = p.local === e.id ? p.visitante : p.local;
        (p.jug || []).forEach(function (fila) {
          if (!fila.j) return;
          if (fila.e === e.id) {
  ...
  // Ranking final de una categoría: texto pegado (IA) + auto-suma humana,
  // top 15 — mismo criterio que calcularUscStatsCombinado, sobre el
  // almacén propio de Supercopa de España (Estado.obtenerSceStatTexto).
  function calcularSceStatsCombinado(datos, categoria) {
    var texto = window.Estado ? window.Estado.obtenerSceStatTexto(categoria) : "";
    var filas = _fusionarStatFilasConOverride(
      parsearLiga1RefStatTexto(texto),
      calcularSceStatsHumanos(datos)[categoria] || []
    );
  ...
  // Bloque "estado de la Supercopa de España" de UN club — mismo cálculo
  // EXACTO que _uscEstadoClub (reutiliza el mismo _estadoRondasEliminacion,
  // genérico por competición). Devuelve null si el club no tiene ningún
  // partido de esta competición todavía.
  function _sceEstadoClub(datos, e) {
    var partidos = _scePartidosDelClub(datos, e.id);
    if (!partidos.length) return null;
    var ultima = partidos[partidos.length - 1];
    var estado = _estadoRondasEliminacion(partidos, e.id);
    return {
  ...
  // Texto EXACTO del ℹ️ de la Supercopa de España — 2 rondas, TODAS a
  // partido único con prórroga y penaltis (nunca ida+vuelta).
  var FORMATO_SCE_TEXTO = [
    "📋FORMATO SUPERCOPA DE ESPAÑA:",
    "4 clubes participan en 2 eliminatorias: Semifinales y Final.",
    "",
    "👥️FORMATO ELIMINATORIAS (todas a partido único, con prórroga y penaltis en caso de empate):",
    "Semifinales — 2 vs 2.",
  ...
  function obtenerFormatoSceTexto() {
    // Clave "sce" (no "supercopa"): tiene que coincidir con la clave que
    // usa mostrarInfoSce/_abrirInfoOverlay en main.js al GUARDAR el
    // override — "supercopa" es el compKey real de los partidos
    // (Calendario extra), pero el override de este texto informativo es
    // un dato APARTE, indexado por su propia clave corta (mismo patrón
    // que obtenerFormatoUscTexto usa "usc").
    var override = window.Estado ? window.Estado.obtenerFormatoOverride("sce") : "";
    return override || FORMATO_SCE_TEXTO;
  }

  // Las 4 cajas de estadísticas — IDÉNTICAS estén en la pestaña que estén,
  // pintadas al final de LAS 2 (👥️ Humanos y ⛓️ Eliminatorias) — mismo
  // criterio EXACTO que _uscAppendStatsGrid.
  var SCE_STATS = [
    { key: "pichichi", icono: "⚽", label: "PICHICHI", columna: "Goles" },
    { key: "mvp", icono: "⭐", label: "MVP", columna: "MVP" },
    { key: "amarillas", icono: "🟨", label: "T. AMARILLAS", columna: "Amarillas" },
  ...
  function _sceAppendStatsGrid(contenedor, idClubActivo) {
    contenedor.appendChild(nodoSeparador());
    contenedor.appendChild(nodoTituloEstadisticas());

    var statsGrid = document.createElement("div");
    statsGrid.className = "liga1ref-stats-grid";
    statsGrid.innerHTML = SCE_STATS.map(function (s) {
      return '<button type="button" class="liga1ref-stat-box" data-accion="ver-sce-stat" data-club-id="' +
        (idClubActivo || "") + '" data-categoria="' + s.key + '"><span class="liga1ref-stat-box-icono">' +
        s.icono + '</span><span class="liga1ref-stat-box-label">' + escapeHTML(s.label) + "</span></button>";
    }).join("");
    contenedor.appendChild(statsGrid);
  ...
  // ---------- Pestañas 👥️ Humanos / ⛓️ Eliminatorias — mismo patrón
  // exacto que las pestañas de Supercopa de Europa/Copa del Rey ----------
  var _sceTabActual = "humanos"; // "humanos" | "eliminatorias" — no se persiste, siempre reabre en Humanos

  function _sceTabsHTML(idClubActivo) {
    return (
      '<div class="liga-tab-boxes copa-tabs">' +
      '<button type="button" class="liga-tab-box liga-tab-box--copa-humanos' +
      (_sceTabActual === "humanos" ? " liga-tab-box--activa" : "") +
      '" data-accion="sce-tab-ir" data-tab="humanos" data-club-id="' + (idClubActivo || "") + '">👥️ Humanos</button>' +
      '<button type="button" class="liga-tab-box liga-tab-box--copa-eliminatorias' +
      (_sceTabActual === "eliminatorias" ? " liga-tab-box--activa" : "") +
      '" data-accion="sce-tab-ir" data-tab="eliminatorias" data-club-id="' + (idClubActivo || "") + '">⛓️ Eliminatorias</button>' +
      "</div>"
    );
  }
  ...
  // Pestaña 👥️ Humanos — el cuadro completo (Semifinal → Final) de CADA
  // club humano, uno debajo del otro — mismo patrón EXACTO que
  // _renderizarUscHumanos, reutilizando _copaBloqueClubHTML tal cual.
  function _renderizarSceHumanos(contenedor, datos, idClubActivo) {
    var equiposHumanos = _sceEquiposHumanos(datos).slice().sort(function (a, b) {
      if (a.id === idClubActivo) return -1;
      if (b.id === idClubActivo) return 1;
      return a.nombre.localeCompare(b.nombre);
    });
    ...
    var bloques = equiposHumanos
      .map(function (e) { return _sceEstadoClub(datos, e); })
      .filter(Boolean);

    if (!bloques.length) {
      contenedor.appendChild(nodoEstado("🏅", "Todavía no hay partidos de la Supercopa de España. Añádelos desde el ✏️ de cada caja (Calendario extra → Competición «Supercopa España»)."));
    } else {
      bloques.forEach(function (b, bi) {
        if (bi > 0) contenedor.appendChild(nodoSeparador());
        contenedor.insertAdjacentHTML("beforeend", _copaBloqueClubHTML(b, datos, idClubActivo));
    ...
    _sceAppendStatsGrid(contenedor, idClubActivo);
  }

  // ---------- Pestaña ⛓️ Eliminatorias — cuadro ÚNICO de 2 rondas ----------
  // Semifinal YA es la primera ronda del cuadro de 4 clubes — las 2
  // rondas se ven todas aquí, igual que Supercopa de Europa.
  var SCE_PLAYOFF_RONDAS = [
    { key: "semis", label: "Semifinales" },
    { key: "final", label: "Final" }
  ];

  // Cruce AUTO-COMPUTADO de un club humano en UNA ronda concreta — mismo
  // criterio EXACTO que _uscPlayoffHumano, filtrando competicion==="supercopa"
  // y reutilizando el detector de ronda de Copa (_copaPlayoffRondaCoincide,
  // genérico por rondaKey/rondaTexto — no depende de qué competición sea).
  function _scePlayoffHumano(datos, club, rondaKey) {
    var partidos = (window.Estado ? window.Estado.listarPartidosResueltos(datos) : []).filter(function (p) {
      if (!(p.jugado && p.resultado && p.competicion === "supercopa" && (p.local === club.id || p.visitante === club.id))) return false;
      return _copaPlayoffRondaCoincide(rondaKey, p.ronda || "");
    });
    if (!partidos.length) return null;
    var rivalId = null, golA = 0, golB = 0;
  ...
  // Los cruces de UNA ronda de la Supercopa de España (Semifinal o Final —
  // AMBAS se resuelven IGUAL, ninguna depende de la anterior) — mismo
  // criterio EXACTO que _uscRondaDesdeTexto: cada club humano con
  // partidos YA jugados esta ronda aporta el suyo auto-computado; el resto
  // sale del texto libre que el admin pega para ESA ronda concreta
  // ("Equipo A 3-1 Equipo B", parsearChampionsPlayoffTexto).
  function _sceRondaDesdeTexto(datos, rondaKey) {
    var equiposHumanos = _sceEquiposHumanos(datos);
    var ties = [];
    var indexPorHumano = {};

    equiposHumanos.forEach(function (e) {
      var t = _scePlayoffHumano(datos, e, rondaKey);
      if (!t) return;
      indexPorHumano[e.id] = ties.length;
      ties.push({
        equipoA: t.equipoA, equipoAObj: t.equipoAObj, golA: t.golA, golB: t.golB,
    ...
    var texto = window.Estado ? window.Estado.obtenerScePlayoffTexto(rondaKey) : "";
    parsearChampionsPlayoffTexto(texto).forEach(function (it) {
      var objA = resolverRivalPorNombre(it.equipoA, datos, null);
      var objB = resolverRivalPorNombre(it.equipoB, datos, null);
      var idxHumanoA = objA && objA.mister ? indexPorHumano[objA.id] : undefined;
  ...
  // INDEPENDIENTES entre sí, cada una sale directamente del texto que el
  // admin pegó para ESA ronda.
  function calcularScePlayoffTodasLasRondas(datos) {
    return {
      semis: _sceRondaDesdeTexto(datos, "semis"),
      final: _sceRondaDesdeTexto(datos, "final")
    };
  }

  function _renderizarSceEliminatorias(contenedor, datos, idClubActivo) {
    var rondas = calcularScePlayoffTodasLasRondas(datos);

    SCE_PLAYOFF_RONDAS.forEach(function (meta) {
      var ties = rondas[meta.key] || [];
      var bloque = document.createElement("div");
      bloque.className = "copa-club-block champions-playoff-ronda";
      var header =
        '<div class="copa-club-header">' +
        '<span class="copa-club-nombre">' + escapeHTML(meta.label) + "</span>" +
        '<button type="button" class="liga1ref-editar-btn" data-accion="editar-sce-playoff-inline" data-ronda="' +
        meta.key + '" data-club-id="' + (idClubActivo || "") + '" aria-label="Editar ' + escapeHTML(meta.label) + '">✏️</button>' +
        "</div>";
      var cuerpo;
      if (!ties.length) {
  ...
  function renderizarSce(contenedorId, idClubActivo) {
    var contenedor = document.getElementById(contenedorId);
    if (!contenedor) return;
    contenedor.innerHTML = "";
    contenedor.appendChild(nodoEstado("⏳", "Cargando…"));
    ...
    cargarTodo().then(function (datos) {
      contenedor.innerHTML = "";
      contenedor.insertAdjacentHTML("beforeend", _sceTabsHTML(idClubActivo));

      if (_sceTabActual === "eliminatorias") _renderizarSceEliminatorias(contenedor, datos, idClubActivo);
      else _renderizarSceHumanos(contenedor, datos, idClubActivo);
    });
  }

  // Cambia de pestaña (👥️ Humanos ⇄ ⛓️ Eliminatorias) y re-pinta — único
  // punto que toca `_sceTabActual`, mismo patrón que irUscTab.
  function irSceTab(idClubActivo, tab) {
    _sceTabActual = tab === "eliminatorias" ? "eliminatorias" : "humanos";
    renderizarSce("sce-content", idClubActivo);
  }

  // Ranking (top 15) de UNA categoría de Supercopa de España — mismo
  // patrón exacto que renderizarUscStatDetalle.
  function renderizarSceStatDetalle(contenedorId, idClubActivo, categoria) {
    var contenedor = document.getElementById(contenedorId);
    if (!contenedor) return;
    var meta = SCE_STATS.filter(function (s) { return s.key === categoria; })[0];
    if (!meta) return;
    contenedor.innerHTML = "";
    contenedor.appendChild(nodoEstado("⏳", "Cargando…"));
    ...
    cargarTodo().then(function (datos) {
      ...
      header.className = "liga1ref-header";
      header.innerHTML =
        '<button type="button" class="btn-ghost liga1ref-volver-btn" data-accion="volver-sce" data-club-id="' +
        (idClubActivo || "") + '">← Volver</button>' +
        '<button type="button" class="liga1ref-editar-btn" data-accion="editar-sce-stat-inline" data-club-id="' +
        (idClubActivo || "") + '" data-categoria="' + categoria + '" aria-label="Editar ' + escapeHTML(meta.label) + '">✏️</button>';
      contenedor.appendChild(header);
      ...
      var filas = calcularSceStatsCombinado(datos, categoria);
      if (!filas.length) {
        contenedor.appendChild(nodoEstado(meta.icono, "Todavía no hay datos. Pulsa ✏️ para añadirlos, o suman solos al añadir eventos de un club humano."));
        return;
      }
      ...
      filas.forEach(function (f, i) {
        ...
        tr.innerHTML =
          ...
          '<td class="liga1ref-stat-equipo">' + escapeHTML(f.equipo || "—") + "</td>" +
          '<td class="clasificacion-pts">' + f.cantidad + "</td>" +
          '<td class="stat-fila-editar-td">' + _statFilaEditarBtnHtml("sce", "", categoria, idClubActivo, f, meta) + "</td>";
        tbody.appendChild(tr);
      });
      tablaEl.appendChild(tbody);
      wrap.appendChild(tablaEl);
  ...
  // Editor inline de UNA categoría de estadística de la Supercopa de
  // España (PIN 646) — mismo patrón exacto que pintarEditorUscStat.
  function pintarEditorSceStat(contenedor, idClubActivo, categoria) {
    var meta = SCE_STATS.filter(function (s) { return s.key === categoria; })[0];
    if (!meta) return;
    contenedor.innerHTML = "";
    ...
    nota.textContent =
      "Pega el ranking, una línea por jugador: «Nombre Jugador - Equipo  " + meta.columna +
      "» (el Nº inicial es opcional, se recalcula solo). Los jugadores de las 6 cajas " +
      "humanas se suman SOLOS al añadir eventos en un partido de Supercopa de España — no hace falta escribirlos aquí, " +
      "salvo que el cálculo automático no cuadre: si escribes el nombre EXACTO de un jugador que la " +
      "app ya calcula sola (humano o rival IA con ficha real), tu línea CORRIGE ese número en vez de " +
      "sumarse aparte.";
    contenedor.appendChild(nota);
    ...
    var textarea = document.createElement("textarea");
    textarea.id = "sce-stat-textarea";
    textarea.className = "admin-roadmap-textarea";
    textarea.rows = 14;
    textarea.placeholder = "1º Mohamed Salah - Liverpool  3\n2º Bukayo Saka - Arsenal  2";
    textarea.value = window.Estado ? window.Estado.obtenerSceStatTexto(categoria) : "";
    contenedor.appendChild(textarea);

    var acciones = document.createElement("div");
    acciones.className = "admin-roadmap-editor-acciones";
    acciones.innerHTML =
      '<button type="button" class="btn-ghost" data-accion="cancelar-sce-stat" data-club-id="' + (idClubActivo || "") + '" data-categoria="' + categoria + '">✕ Cancelar</button>' +
      '<button type="button" class="admin-list-add-btn" data-accion="guardar-sce-stat" data-club-id="' + (idClubActivo || "") + '" data-categoria="' + categoria + '">💾 Guardar</button>';
    contenedor.appendChild(acciones);
  }

  // Editor inline de UNA ronda del cuadro ⛓️ Eliminatorias (PIN 646) —
  // mismo patrón exacto que pintarEditorUscPlayoff: las 2 rondas se
  // pegan igual, una línea por cruce, siempre marcador de partido único.
  function pintarEditorScePlayoff(contenedor, idClubActivo, rondaKey) {
    var meta = SCE_PLAYOFF_RONDAS.filter(function (r) { return r.key === rondaKey; })[0];
    if (!meta) return;
    var esSemis = rondaKey === "semis";
    contenedor.innerHTML = "";
    ...
    nota.textContent = esSemis
      ? "Una línea por cruce, pega el marcador (partido único, con prórroga/penaltis si hizo falta) TAL CUAL " +
        "lo veas en el juego — vale «-» o «vs»: «Liverpool 2-1 Real Madrid» o «Real Madrid 1 vs 2 Liverpool», da " +
        "igual el orden. Usa los nombres TAL CUAL salen en el resto de la app. Si un club humano ya tiene su " +
        "cruce jugado (Calendario extra), esta línea se ignora — su resultado real manda siempre."
      : "Pega el marcador (partido único, con prórroga/penaltis si hizo falta) del cruce de la Final — vale «-» " +
        "o «vs»: «Equipo A 3-1 Equipo B» o «Equipo A 3 vs 1 Equipo B». Si un club humano ya tiene su propio " +
        "partido de Supercopa de España jugado en esta ronda (Calendario extra), su cruce se calcula solo y " +
        "esta línea se ignora.";
    contenedor.appendChild(nota);

    var textarea = document.createElement("textarea");
    textarea.id = "sce-playoff-textarea";
    textarea.className = "admin-roadmap-textarea";
    textarea.rows = esSemis ? 6 : 4;
    textarea.placeholder = esSemis
      ? "Liverpool 2-1 Real Madrid\nArsenal 3 vs 0 FC Barcelona"
      : "Liverpool 1-0 Arsenal";
    textarea.value = window.Estado ? window.Estado.obtenerScePlayoffTexto(rondaKey) : "";
    contenedor.appendChild(textarea);

    var acciones = document.createElement("div");
    acciones.className = "admin-roadmap-editor-acciones";
    acciones.innerHTML =
      '<button type="button" class="btn-ghost" data-accion="cancelar-sce-playoff" data-club-id="' + (idClubActivo || "") + '" data-ronda="' + rondaKey + '">✕ Cancelar</button>' +
      '<button type="button" class="admin-list-add-btn" data-accion="guardar-sce-playoff" data-club-id="' + (idClubActivo || "") + '" data-ronda="' + rondaKey + '">💾 Guardar</button>';
    contenedor.appendChild(acciones);
  }
  ...
  // y perdería la tanda) — ver _championsTieRowHTML para el desempate.
  ...
  // Cruces de UNA ronda de Champions a partir de Octavos (Octavos,
  // Cuartos, Semis, Final — LAS 4 se resuelven IGUAL, ninguna depende de
  // la anterior): cada club humano con partidos YA jugados esta ronda
  // aporta el suyo auto-computado; el resto sale ÍNTEGRAMENTE del texto
  // libre que el admin pega para ESA ronda ("Equipo A 3-1 Equipo B",
  // parsearChampionsPlayoffTexto). PROHIBIDO inferir el emparejamiento
  // de Octavos+ a partir del ganador/espera de la ronda anterior —
  // petición usuario 2026-09-04: "los cruces los doy yo en texto, elimina
  // el algoritmo de generarlos automáticamente" (se reportó con Octavos
  // generándose solo a partir de los ganadores/espera de Dieciseisavos).
  // El "espera" de Dieciseisavos (`_championsDieciseisavosTies`) queda
  // como anotación de SOLO DISPLAY — no alimenta esta función.
  function _championsRondaDesdeTexto(datos, rondaKey) {
    var equiposHumanos = _championsEquiposHumanos(datos);
    var ties = [];
    var indexPorHumano = {};

    equiposHumanos.forEach(function (e) {
      var t = _championsPlayoffHumano(datos, e, rondaKey);
      if (!t) return;
      indexPorHumano[e.id] = ties.length;
      ties.push({
        equipoA: t.equipoA, equipoAObj: t.equipoAObj, golA: t.golA, golB: t.golB,
  ...
  // Punto ÚNICO que calcula el bracket ENTERO — las 5 rondas son
  // INDEPENDIENTES entre sí, cada una sale directamente del texto que
  // el admin pegó para ESA ronda (nunca del ganador de la anterior).
  function calcularChampionsPlayoffTodasLasRondas(datos) {
    return {
      dieciseisavos: _championsDieciseisavosTies(datos),
      octavos: _championsRondaDesdeTexto(datos, "octavos"),
      cuartos: _championsRondaDesdeTexto(datos, "cuartos"),
      semis: _championsRondaDesdeTexto(datos, "semis"),
      final: _championsRondaDesdeTexto(datos, "final")
    };
  }
  ...
  // ============================================================
  // 3c-octies. CONFERENCE LEAGUE — MISMO FORMATO EXACTO que Champions y
  // Europa League (petición usuario: "Creame la Conference, idéntica a
  // Champions y Europa League con sus colores en verde"): 👥️ Humanos +
  // 🟢 Fase Grupos (40 equipos, "la batidora") + 🟩 Playoffs
  // (Dieciseisavos → Octavos → Cuartos → Semis → Final), mismas 4 cajas
  // de estadísticas. Es una réplica 1:1 de la sección de Europa League
  // de arriba (que a su vez ya lo es de Champions) — MISMA lógica,
  // filtrando competicion==="uecl" en vez de "uel"/"champions" (mismo
  // compKey de 4 letras que ya usa el resto del proyecto para Conference
  // League — ver COMP_LABEL/COMP_CLASE/_BALON_COMP_ALIAS más arriba,
  // así que un partido de "Calendario extra" con Competición «Conference
  // League»/«UECL» ya resuelve aquí sin tocar nada más) — reutilizando
  // además, sin duplicarlos, los parsers/pintores 100% genéricos que ya
  // usa Champions/Europa League Y Copa del Rey
  // (parsearChampionsPlayoffTexto, parsearChampionsDieciseisavosTexto,
  // _championsTieRowHTML, _construirTbodyClasificacion): ninguno de esos
  // 4 tiene nada específico de "Champions" en su lógica, operan sobre
  // "tie"/"fila" genéricos — es el mismo criterio ya aplicado por Copa
  // del Rey/Europa League al reutilizarlos para su propio cuadro.
  // ============================================================
  var UECL_STATS = [
    { key: "pichichi", icono: "⚽", label: "PICHICHI", columna: "Goles" },
    { key: "mvp", icono: "⭐", label: "MVP", columna: "MVP" },
  ...
  // Ningún club humano queda excluido de Conference League — a diferencia de
  // Copa del Rey (PSG juega su propia Coupe de France), la Conference League
  // es una competición europea que cualquiera de los 6 puede clasificar
  // a jugar, igual que Champions.
  function _ueclEquiposHumanos(datos) {
    return datos.equipos.equipos || [];
  }
  ...
  // La fusión: texto pegado (solo IA) + los 6 clubes humanos con sus
  // propios partidos de competicion==="uecl" ya jugados — mismo criterio
  // EXACTO que calcularChampionsCombinada.
  function calcularUeclCombinada(datos) {
    var equiposHumanos = _ueclEquiposHumanos(datos);
    var idsHumanos = {};
    equiposHumanos.forEach(function (e) { idsHumanos[e.id] = true; });
    var filas = [];

    var texto = window.Estado ? window.Estado.obtenerUeclTexto() : "";
    parsearLiga1RefTexto(texto).forEach(function (f) {
      if (_liga1RefEsNombreHumano(f.nombre, equiposHumanos)) return; // su fila la aporta el bloque de abajo
      filas.push({
        nombre: f.nombre, nombreMostrado: f.nombre, equipoId: null,
    ...
    equiposHumanos.forEach(function (e) {
      // Igual que Champions: la Conference League NO garantiza que un club
      // humano esté clasificado esta temporada — un club solo aporta fila
      // si tiene AL MENOS un partido (jugado o programado) registrado.
      var partidosComp = todosPartidos.filter(function (p) {
        return p.competicion === "uecl" && (p.local === e.id || p.visitante === e.id);
      });
      if (!partidosComp.length) return;
  ...
  // criterio EXACTO que _championsPlayoffHumano, filtrando
  // competicion==="uecl".
  function _ueclPlayoffHumano(datos, club, rondaKey) {
    var re = CHAMPIONS_PLAYOFF_REGEX_POR_KEY[rondaKey];
    if (!re) return null;
    var partidos = (window.Estado ? window.Estado.listarPartidosResueltos(datos) : []).filter(function (p) {
      if (!(p.jugado && p.resultado && p.competicion === "uecl" && (p.local === club.id || p.visitante === club.id))) return false;
      var t = _normNombre(p.ronda || "");
      if (rondaKey === "final" && /\bsemi/.test(t)) return false;
      return re.test(t);
    });
  ...
  // Los 8 cruces de Dieciseisavos — mismo criterio EXACTO que
  // _championsDieciseisavosTies, con la clave de almacenamiento propia
  // de Conference League (Estado.obtenerUeclPlayoffTexto).
  function _ueclDieciseisavosTies(datos) {
    var equiposHumanos = _ueclEquiposHumanos(datos);
    var ties = [];
    var indexPorHumano = {};

    equiposHumanos.forEach(function (e) {
      var t = _ueclPlayoffHumano(datos, e, "dieciseisavos");
      if (!t) return;
      indexPorHumano[e.id] = ties.length;
      ties.push({
        equipoA: t.equipoA, equipoAObj: t.equipoAObj, golA: t.golA, golB: t.golB,
    ...
    var texto = window.Estado ? window.Estado.obtenerUeclPlayoffTexto("dieciseisavos") : "";
    parsearChampionsDieciseisavosTexto(texto).forEach(function (it) {
      var objA = resolverRivalPorNombre(it.equipoA, datos, null);
      var objB = resolverRivalPorNombre(it.equipoB, datos, null);
      var objEspera = it.espera ? resolverRivalPorNombre(it.espera, datos, null) : null;
  ...
  // esta ronda aporta el suyo auto-computado, el resto sale del texto
  // libre pegado por el admin para ESA ronda.
  function _ueclRondaDesdeTexto(datos, rondaKey) {
    var equiposHumanos = _ueclEquiposHumanos(datos);
    var ties = [];
    var indexPorHumano = {};

    equiposHumanos.forEach(function (e) {
      var t = _ueclPlayoffHumano(datos, e, rondaKey);
      if (!t) return;
      indexPorHumano[e.id] = ties.length;
      ties.push({
        equipoA: t.equipoA, equipoAObj: t.equipoAObj, golA: t.golA, golB: t.golB,
    ...
    var texto = window.Estado ? window.Estado.obtenerUeclPlayoffTexto(rondaKey) : "";
    parsearChampionsPlayoffTexto(texto).forEach(function (it) {
      var objA = resolverRivalPorNombre(it.equipoA, datos, null);
      var objB = resolverRivalPorNombre(it.equipoB, datos, null);
      var idxHumanoA = objA && objA.mister ? indexPorHumano[objA.id] : undefined;
  ...
  // Punto ÚNICO que calcula el bracket ENTERO de Conference League — las 5
  // rondas son INDEPENDIENTES entre sí, mismo criterio que Champions.
  function calcularUeclPlayoffTodasLasRondas(datos) {
    return {
      dieciseisavos: _ueclDieciseisavosTies(datos),
      octavos: _ueclRondaDesdeTexto(datos, "octavos"),
      cuartos: _ueclRondaDesdeTexto(datos, "cuartos"),
      semis: _ueclRondaDesdeTexto(datos, "semis"),
      final: _ueclRondaDesdeTexto(datos, "final")
    };
  }

  var _ueclTabActual = "humanos"; // "humanos" | "grupos" | "playoffs" — no se persiste, siempre reabre en Humanos

  function _ueclTabsHTML(idClubActivo) {
    return (
      '<div class="liga-tab-boxes uecl-tabs">' +
      '<button type="button" class="liga-tab-box liga-tab-box--uecl-humanos' +
      (_ueclTabActual === "humanos" ? " liga-tab-box--activa" : "") +
  ...
  // Partidos de Conference League de UN club, ordenados por fecha — mismo
  // criterio EXACTO que _championsPartidosDelClub.
  function _ueclPartidosDelClub(datos, clubId) {
    return (window.Estado ? window.Estado.listarPartidosResueltos(datos) : [])
      .filter(function (p) {
        return p.competicion === "uecl" && (p.local === clubId || p.visitante === clubId);
      })
      .sort(function (a, b) {
        var ta = a.fecha ? new Date(a.fecha).getTime() : (a._fechaFallbackMs || 0);
        var tb = b.fecha ? new Date(b.fecha).getTime() : (b._fechaFallbackMs || 0);
  ...
  function _ueclEstadoClub(datos, e) {
    var partidos = _ueclPartidosDelClub(datos, e.id);
    if (!partidos.length) return null;
    var ultima = partidos[partidos.length - 1];
    return { equipo: e, partidos: partidos, rondaActual: ultima.ronda || "—" };
  }

  // Fila de UN partido de Conference League de un club — mismo layout/clases
  // EXACTOS que _championsPartidoRowHTML.
  function _ueclPartidoRowHTML(p, clubId, datos) {
    var esLocal = p.local === clubId;
    var rival = buscarEquipoPorId(esLocal ? p.visitante : p.local, datos);
    var esBloqueado = !!(rival && rival.desconocido);
    var claseEstado = "";
  ...
  function _renderizarUeclHumanos(contenedor, datos, idClubActivo) {
    var equiposHumanos = _ueclEquiposHumanos(datos).slice().sort(function (a, b) {
      if (a.id === idClubActivo) return -1;
      if (b.id === idClubActivo) return 1;
      return a.nombre.localeCompare(b.nombre);
    });
    ...
    var bloques = equiposHumanos
      .map(function (e) { return _ueclEstadoClub(datos, e); })
      .filter(Boolean);

    if (!bloques.length) {
      contenedor.appendChild(nodoEstado("🟢", "Todavía no hay partidos de Conference League. Añádelos desde el ✏️ de cada caja (Calendario extra → Competición «Conference League»)."));
    } else {
      bloques.forEach(function (b, bi) {
        if (bi > 0) contenedor.appendChild(nodoSeparador());
        var esActivo = b.equipo.id === idClubActivo;
        ...
        bloque.innerHTML =
          ...
          "</div>" +
          '<div class="copa-club-partidos">' +
          b.partidos.map(function (p) { return _ueclPartidoRowHTML(p, b.equipo.id, datos); }).join("") +
          "</div>";
        contenedor.appendChild(bloque);
      });
    }

    _ueclAppendStatsGrid(contenedor, idClubActivo);
  }

  // Las 4 cajas de estadísticas — mismo criterio EXACTO que
  // _championsAppendStatsGrid: idénticas en las 3 pestañas.
  function _ueclAppendStatsGrid(contenedor, idClubActivo) {
    contenedor.appendChild(nodoSeparador());
    contenedor.appendChild(nodoTituloEstadisticas());

    var statsGrid = document.createElement("div");
    statsGrid.className = "liga1ref-stats-grid";
    statsGrid.innerHTML = UECL_STATS.map(function (s) {
      return '<button type="button" class="liga1ref-stat-box" data-accion="ver-uecl-stat" data-club-id="' +
        (idClubActivo || "") + '" data-categoria="' + s.key + '"><span class="liga1ref-stat-box-icono">' +
        s.icono + '</span><span class="liga1ref-stat-box-label">' + escapeHTML(s.label) + "</span></button>";
    }).join("");
    contenedor.appendChild(statsGrid);
  ...
  function _renderizarUeclGrupos(contenedor, datos, idClubActivo) {
    var filas = calcularUeclCombinada(datos);

    var tituloRow = document.createElement("div");
    tituloRow.className = "liga1ref-titulo-row";
    tituloRow.innerHTML =
      '<span class="liga1ref-titulo-spacer"></span>' +
      '<span class="liga1ref-titulo-actual">🟢 Conference League · Fase de Grupos</span>' +
      '<button type="button" class="liga1ref-editar-btn" data-accion="editar-uecl-inline" data-club-id="' +
      (idClubActivo || "") + '" aria-label="Editar clasificación">✏️</button>';
    contenedor.appendChild(tituloRow);
  ...
  function _renderizarUeclPlayoffs(contenedor, datos, idClubActivo) {
    var rondas = calcularUeclPlayoffTodasLasRondas(datos);

    UECL_PLAYOFF_RONDAS.forEach(function (meta) {
      var ties = rondas[meta.key] || [];
      var bloque = document.createElement("div");
      bloque.className = "copa-club-block champions-playoff-ronda";
      var header =
        '<div class="copa-club-header">' +
        '<span class="copa-club-nombre">' + escapeHTML(meta.label) + "</span>" +
        '<button type="button" class="liga1ref-editar-btn" data-accion="editar-uecl-playoff-inline" data-ronda="' +
        meta.key + '" data-club-id="' + (idClubActivo || "") + '" aria-label="Editar ' + escapeHTML(meta.label) + '">✏️</button>' +
        "</div>";
      var cuerpo;
      if (!ties.length) {
  ...
  function renderizarUecl(contenedorId, idClubActivo) {
    var contenedor = document.getElementById(contenedorId);
    if (!contenedor) return;
    contenedor.innerHTML = "";
    contenedor.appendChild(nodoEstado("⏳", "Cargando…"));
    ...
    cargarTodo().then(function (datos) {
      contenedor.innerHTML = "";
      contenedor.insertAdjacentHTML("beforeend", _ueclTabsHTML(idClubActivo));

      if (_ueclTabActual === "playoffs") _renderizarUeclPlayoffs(contenedor, datos, idClubActivo);
      else if (_ueclTabActual === "grupos") _renderizarUeclGrupos(contenedor, datos, idClubActivo);
      else _renderizarUeclHumanos(contenedor, datos, idClubActivo);
    });
  }

  function irUeclTab(idClubActivo, tab) {
    _ueclTabActual = (tab === "playoffs" || tab === "grupos") ? tab : "humanos";
    renderizarUecl("uecl-content", idClubActivo);
  }

  function renderizarUeclStatDetalle(contenedorId, idClubActivo, categoria) {
    var contenedor = document.getElementById(contenedorId);
    if (!contenedor) return;
    var meta = UECL_STATS.filter(function (s) { return s.key === categoria; })[0];
    if (!meta) return;
    contenedor.innerHTML = "";
    contenedor.appendChild(nodoEstado("⏳", "Cargando…"));
    ...
    cargarTodo().then(function (datos) {
      ...
      header.className = "liga1ref-header";
      header.innerHTML =
        '<button type="button" class="btn-ghost liga1ref-volver-btn" data-accion="volver-uecl" data-club-id="' +
        (idClubActivo || "") + '">← Volver</button>' +
        '<button type="button" class="liga1ref-editar-btn" data-accion="editar-uecl-stat-inline" data-club-id="' +
        (idClubActivo || "") + '" data-categoria="' + categoria + '" aria-label="Editar ' + escapeHTML(meta.label) + '">✏️</button>';
      contenedor.appendChild(header);
      ...
      var filas = calcularUeclStatsCombinado(datos, categoria);
      if (!filas.length) {
        contenedor.appendChild(nodoEstado(meta.icono, "Todavía no hay datos. Pulsa ✏️ para añadirlos, o suman solos al añadir eventos de un club humano."));
        return;
      }
  ...
  function pintarEditorUecl(contenedor, idClubActivo) {
    contenedor.innerHTML = "";
    ...
    var textarea = document.createElement("textarea");
    textarea.id = "uecl-textarea";
    textarea.className = "admin-roadmap-textarea";
    textarea.rows = 16;
    textarea.placeholder = "1  Liverpool       18  6  0  0  20  2   18\n2  AS Roma         15  6  1  1  18  8   10";
    textarea.value = window.Estado ? window.Estado.obtenerUeclTexto() : "";
    contenedor.appendChild(textarea);

    var acciones = document.createElement("div");
    acciones.className = "admin-roadmap-editor-acciones";
    acciones.innerHTML =
      '<button type="button" class="btn-ghost" data-accion="cancelar-uecl" data-club-id="' + (idClubActivo || "") + '">✕ Cancelar</button>' +
      '<button type="button" class="admin-list-add-btn" data-accion="guardar-uecl" data-club-id="' + (idClubActivo || "") + '">💾 Guardar</button>';
    contenedor.appendChild(acciones);
  }

  function pintarEditorUeclStat(contenedor, idClubActivo, categoria) {
    var meta = UECL_STATS.filter(function (s) { return s.key === categoria; })[0];
    if (!meta) return;
    contenedor.innerHTML = "";
    ...
    var textarea = document.createElement("textarea");
    textarea.id = "uecl-stat-textarea";
    textarea.className = "admin-roadmap-textarea";
    textarea.rows = 14;
    textarea.placeholder = "1º Mohamed Salah - Liverpool  9\n2º Paulo Dybala - AS Roma  7";
    textarea.value = window.Estado ? window.Estado.obtenerUeclStatTexto(categoria) : "";
    contenedor.appendChild(textarea);

    var acciones = document.createElement("div");
    acciones.className = "admin-roadmap-editor-acciones";
    acciones.innerHTML =
      '<button type="button" class="btn-ghost" data-accion="cancelar-uecl-stat" data-club-id="' + (idClubActivo || "") + '" data-categoria="' + categoria + '">✕ Cancelar</button>' +
      '<button type="button" class="admin-list-add-btn" data-accion="guardar-uecl-stat" data-club-id="' + (idClubActivo || "") + '" data-categoria="' + categoria + '">💾 Guardar</button>';
    contenedor.appendChild(acciones);
  }

  function pintarEditorUeclPlayoff(contenedor, idClubActivo, rondaKey) {
    var meta = UECL_PLAYOFF_RONDAS.filter(function (r) { return r.key === rondaKey; })[0];
    if (!meta) return;
    var esDieciseisavos = rondaKey === "dieciseisavos";
    contenedor.innerHTML = "";
    ...
    nota.textContent = esDieciseisavos
      ...
      : "Pega el marcador AGREGADO (ida+vuelta ya sumada) de cada cruce, una línea por eliminatoria — " +
        "vale «-» o «vs»: «Equipo A 3-1 Equipo B» o «Equipo A 3 vs 1 Equipo B». Los cruces de esta ronda " +
        "salen SOLOS de la ronda anterior — solo hace falta el marcador. Si un club humano ya tiene sus " +
        "propios partidos de Conference League jugados en esta ronda (Calendario extra), su cruce se calcula solo " +
        "y esta línea se ignora.";
    contenedor.appendChild(nota);

    var textarea = document.createElement("textarea");
    textarea.id = "uecl-playoff-textarea";
    textarea.className = "admin-roadmap-textarea";
    textarea.rows = esDieciseisavos ? 10 : 12;
    textarea.placeholder = esDieciseisavos
      ? "AS Roma 4 vs 5 Liverpool\nGalatasaray 7 vs 5 Villarreal > Sevilla"
      : "Liverpool 4-2 Milan\nAS Roma 3 vs 3 Villarreal";
    textarea.value = window.Estado ? window.Estado.obtenerUeclPlayoffTexto(rondaKey) : "";
    contenedor.appendChild(textarea);

    var acciones = document.createElement("div");
    acciones.className = "admin-roadmap-editor-acciones";
    acciones.innerHTML =
      '<button type="button" class="btn-ghost" data-accion="cancelar-uecl-playoff" data-club-id="' + (idClubActivo || "") + '" data-ronda="' + rondaKey + '">✕ Cancelar</button>' +
      '<button type="button" class="admin-list-add-btn" data-accion="guardar-uecl-playoff" data-club-id="' + (idClubActivo || "") + '" data-ronda="' + rondaKey + '">💾 Guardar</button>';
    contenedor.appendChild(acciones);
  }

  // Texto EXACTO del ℹ️ de Conference League — mismo overlay propio que
  // Champions/Copa/Superliga/Liga 1ª REF (ver js/main.js::mostrarInfoUecl).
  var FORMATO_UECL_TEXTO = [
    "📋FORMATO CONFERENCE LEAGUE:",
    "Fase de Grupos de 40 equipos (liguilla única, todos contra todos por jornadas).",
    "",
    "🏁RESOLUCIÓN FASE DE GRUPOS:",
    " 🟦 Puestos 1º al 8º: Clasificación DIRECTA a Octavos de Final.",
  ...
  function obtenerFormatoUeclTexto() {
    var override = window.Estado ? window.Estado.obtenerFormatoOverride("uecl") : "";
    return override || FORMATO_UECL_TEXTO;
  }
  ...
  // Estado._partidosSuperliga — id 100% estable "superliga-<i>-<j>-<leg>"
  // (índices dentro de datos.equipos.equipos). "superliga" también vive en
  // _BALON_COMP_ALIAS (para que el balón/color de card lo reconozcan si
  // aparece escrito en cualquier lado) — así que si el admin teclea por
  // error/costumbre una línea "Superliga - ..." en el "Calendario extra"
  // GENERAL de su club (el mismo cuadro de texto libre que usa para Liga/
  // Copa/Champions), esa línea TAMBIÉN resuelve `competicion==="superliga"`
  // y, sin este filtro por id, se contaba como un partido de Superliga MÁS
  // — inflando PJ/goles a favor/en contra de la clasificación por encima
  // de los 15 partidos reales de cada club (3 legs × 5 rivales). Bug real
  // reportado por el usuario (fotos Superliga con Pichichi/MVP/Amarillas/
  // Rojas duplicados/incoherentes para Atlético Madrid y FC Barcelona).
  var _RE_SUPERLIGA_ID = /^superliga-\d+-\d+-\d+$/;
  function _superligaPartidosDelClub(datos, clubId) {
    return (window.Estado ? window.Estado.listarPartidosResueltos(datos) : []).filter(function (p) {
      return p.competicion === "superliga" && _RE_SUPERLIGA_ID.test(p.id) &&
        (p.local === clubId || p.visitante === clubId);
    });
  }
  ...
  // Superliga de los 6 humanos (nunca hay rival IA que fusionar). Mismo
  // filtro por id que _superligaPartidosDelClub — ver comentario ahí.
  function calcularSuperliga(datos) {
    var equipos = _superligaEquiposHumanos(datos);
    var partidos = (window.Estado ? window.Estado.listarPartidosResueltos(datos) : []).filter(function (p) {
      return p.competicion === "superliga" && _RE_SUPERLIGA_ID.test(p.id) && p.jugado && p.resultado;
    });
  ...
  // Ranking final de una categoría: 100% auto-suma de los 6 humanos, SIN
  // editor ni corrección manual — Superliga es cerrada (solo los 6
  // humanos, ids de partido 100% estables) y su clasificación/stats
  // deben salir SIEMPRE de los partidos realmente jugados, sin que el
  // admin tenga que revisar/pegar nada partido a partido (petición
  // usuario explícita: "no quiero un editor, yo quiero que lo hagas tú").
  // Top 15, empate -> alfabético. Zamora ordena ascendente (menos goles
  // de media es mejor), igual que Liga 1ª REF.
  ...
  // renderizarCopaStatDetalle, sin botón ✏️ (nada que editar, 100%
  // automático desde los partidos ya jugados).
  ...
  // ============================================================
  // OBJETIVOS DEL CLUB — catálogo ABIERTO de cajas (una por cada línea
  // "# Título" que el admin escriba, en el orden en que aparezcan),
  ...
  // ============================================================
  // Iconos por defecto de las cajas "de fábrica" (las que trae
  // OBJETIVOS_DEFAULT_TEXTO en js/estado.js) — SOLO se usan si Estado no
  // está disponible, o como default de una caja que el admin nunca ha
  // personalizado con su propio icono; la fuente real (editable por el
  // admin, candado 646) es Estado.obtenerObjetivosIconos/
  // guardarObjetivosIconoSeccion, que YA acepta cualquier clave — no
  // solo estas 5. Una caja NUEVA que el admin invente (p.ej. "RECOPA")
  // cae al fallback genérico 🎯 hasta que le ponga su propio icono.
  var OBJETIVOS_ICONOS_DEFAULT = { LIGA: "🏆", COPA: "🎖️", CHAMPIONS: "🇪🇺", SUPERLIGA: "🌟", GLOBALES: "🌍" };
  var OBJETIVOS_ICONO_FALLBACK = "🎯";

  // Alias de la PRIMERA palabra de la cabecera — SOLO para las 2 cajas
  // domésticas (Liga/Copa), cuyo nombre real cambia de club a club (el
  // PSG juega "Ligue 1"/"Coupe [de France]", Manchester City "Premier
  // League"/"FA Cup", Inter "Serie A"/"Coppa Italia"): todas caen en la
  // MISMA caja LIGA/COPA sin tener que reescribir el nombre real de su
  // competición. Se comprueba por PREFIJO con límite de palabra (igual
  // que la clave propia, más abajo), así que tolera texto extra detrás
  // — "Coupe Framce" (con errata) sigue reconociéndose porque empieza
  // por "COUPE", igual que "Ligue 1" empieza por "LIGUE".
  //
  // CUALQUIER OTRA cabecera ("# Champions League", "# Recopa Europa",
  // "# Superliga", o lo que sea) NO necesita estar aquí — abre su
  // PROPIA caja automáticamente por su primera palabra (ver
  // _objetivosClaveDeHeader). Antes "CHAMPIONS" forzaba a "RECOPA" a
  // compartir la MISMA caja (pensado para un club que solo juega una
  // competición europea a la vez) — bug 2026-09 (usuario: "La CHAMPIONS
  // y RECOPA están mezclados"): un club que juega AMBAS a la vez (p.ej.
  // el PSG con Champions League Y Recopa Europa) necesita 2 cajas
  // DISTINTAS, no 1 compartida — el catálogo abierto ya lo resuelve
  // solo, sin alias especial.
  ...
  // Identidad (clave estable) de una cabecera "# Título" ya normalizada
  // a MAYÚSCULAS. Primero prueba los alias de Liga/Copa (arriba); si no
  // casa con ninguno, la clave es la PRIMERA PALABRA del propio título
  // tal cual — así "# Champions League" y "# Champions" son SIEMPRE la
  // misma caja (cambiar el texto de después no pierde el progreso ya
  // marcado), y "# Recopa Europa" abre una caja aparte, independiente,
  // la primera vez que se escriba. Catálogo 100% abierto: no hay límite
  // de cuántas cajas nuevas puede crear el admin.
  function _objetivosClaveDeHeader(norm) {
    for (var k in OBJETIVOS_SECCION_ALIAS_PREFIJO) {
      var alias = OBJETIVOS_SECCION_ALIAS_PREFIJO[k];
      var casa = alias.some(function (a) { return new RegExp("^" + a + "\\b").test(norm); });
      if (casa) return k;
    }
    var mPrimera = norm.match(/^([^\s]+)/);
    return mPrimera ? mPrimera[1] : norm;
  }

  // "# Título" (cualquiera — ver _objetivosClaveDeHeader) abre caja;
  // toda línea siguiente "Texto - N" (N del 1 al 5; cualquier otro
  // valor, o si falta, cae a 1) es un objetivo de esa caja. `clave`
  // identifica el objetivo para el progreso marcado — por sección+texto,
  // así cambiar solo el nº de puntos de una línea no borra el progreso
  // ya tocado. `orden` es el orden real en que aparecen las cabeceras en
  // el texto — las cajas se pintan en ESE orden, no uno fijo.
  //
  // La cabecera admite texto EXTRA tras la palabra clave/alias (p.ej.
  // "# LIGA 1ª REF" o "# COPA DEL REY") y el texto COMPLETO que el
  // admin escriba (con su grafía tal cual, sin tocar mayúsculas — el
  // mayusculeo es solo visual, vía CSS) se guarda como TÍTULO visible
  // de la caja.
  //
  ...
  // escrito, o fuera de 1-5) el objetivo se guarda igual con 1 punto por
  // defecto, en vez de descartarse en silencio como antes.
  function parsearObjetivosTexto(texto) {
    var secciones = {}, etiquetas = {}, orden = [];
    var actual = null;
    (texto || "").split("\n").forEach(function (linea) {
      var l = linea.trim();
      if (!l) return;
      ...
      if (mHeader) {
        ...
        var clave = _objetivosClaveDeHeader(norm);
        actual = clave;
        if (!secciones[clave]) { secciones[clave] = []; orden.push(clave); }
      ...
      if (idx === -1) {
        ...
        puntos = (ptsNum >= 1 && ptsNum <= 5) ? ptsNum : 1;
      }
      if (!objTexto) return;
      secciones[actual].push({ texto: objTexto, puntos: puntos, clave: actual + "::" + objTexto });
    });
    return { secciones: secciones, etiquetas: etiquetas, orden: orden };
  }
  ...
  function calcularObjetivosPuntos(idClubActivo) {
    ...
    parseo.orden.forEach(function (s) {
      (parseo.secciones[s] || []).forEach(function (o) {
        totalPts += o.puntos;
  ...
  function renderizarObjetivos(contenedorId, idClubActivo) {
    ...
    if (!parseo.orden.length) {
      var vacioGlobal = document.createElement("p");
      vacioGlobal.className = "objetivos-vacio";
      vacioGlobal.textContent = 'Sin objetivos todavía. Pulsa ✏️ y añade "# Título" para crear la primera caja.';
      contenedor.appendChild(vacioGlobal);
      return;
    }

    // Catálogo ABIERTO (ver parsearObjetivosTexto): una caja por cada
    // "# Título" que aparezca en el texto, en el orden en que se
    // escribieron — ya no hay una lista fija de 5 nombres.
    parseo.orden.forEach(function (s) {
      ...
      cab.innerHTML =
        ...
        (idClubActivo || "") + '" data-seccion="' + escapeHTML(s) + '" aria-label="Editar icono">' +
        (iconos[s] || OBJETIVOS_ICONOS_DEFAULT[s] || OBJETIVOS_ICONO_FALLBACK) + "</button>" +
        '<span class="objetivos-seccion-nombre">' + escapeHTML(etiquetas[s] || s) + "</span>" +
  ...
  function pintarEditorObjetivos(contenedor, idClubActivo) {
    ...
    nota.className = "admin-nota";
    nota.textContent =
      'Una línea "# Título" abre una caja nueva — pon el que quieras (p.ej. "# Champions League", ' +
      '"# Recopa Europa", "# Superliga"), no hay límite de cuántas ni tienen que llamarse de una forma ' +
      'concreta; se ve SIEMPRE en mayúsculas aunque lo escribas en minúsculas. "# Liga"/"# Ligue 1"/' +
      '"# Premier League"/"# Serie A" van todas a la misma caja Liga, y "# Copa"/"# Coupe ..."/' +
      '"# FA Cup"/"# Coppa Italia" a la misma caja Copa (puedes añadir texto detrás, p.ej. ' +
      '"# Ligue 1 Francia", y se queda como título). Debajo de cada "#", un objetivo por línea con ' +
      '"- N" al final (N del 1 al 5, los puntos que vale); si te dejas el número o el guion mal ' +
      "puesto, el objetivo se guarda igual con 1 punto, nunca desaparece. Añade, edita o borra líneas " +
      "libremente.";
    contenedor.appendChild(nota);
  ...
  // Liga y Copa fuera de la app y solo quiere anotar el resultado final;
  // "el resto con card" — Champions/Supercopa Francia/Supercopa Europa/
  // cualquier otra competición de su calendario usan la card normal con
  // escudos+PREVIA, igual que cualquier otro club humano). En vez del
  // botón PREVIA, la card muestra 3 iconos ✅➖❌ directamente — sin pasar
  // por la pantalla de previa/en vivo. Mapa club → { compKey: true }, con
  // `_resolverCompKeyBalon` de por medio (mismo alias que ya usa el
  ...
  window.Renderizadores = {
    ...
    obtenerFormatoSceTexto: obtenerFormatoSceTexto,
    renderizarSce: renderizarSce,
    irSceTab: irSceTab,
    renderizarSceStatDetalle: renderizarSceStatDetalle,
    pintarEditorSceStat: pintarEditorSceStat,
    pintarEditorScePlayoff: pintarEditorScePlayoff,
    calcularScePlayoffTodasLasRondas: calcularScePlayoffTodasLasRondas,
    ...
    renderizarUecl: renderizarUecl,
    irUeclTab: irUeclTab,
    renderizarUeclStatDetalle: renderizarUeclStatDetalle,
    pintarEditorUecl: pintarEditorUecl,
    pintarEditorUeclStat: pintarEditorUeclStat,
    pintarEditorUeclPlayoff: pintarEditorUeclPlayoff,
    obtenerFormatoUeclTexto: obtenerFormatoUeclTexto,
    calcularUeclCombinada: calcularUeclCombinada,
